...
class TestGeneralLedger(FrappeTestCase):
	...
	def test_get_conditions_branches_TC_ACC_396(self):
		...
		with (
			...
			patch(
				"erpnext.accounts.report.general_ledger.general_ledger.get_dimension_with_children",
				return_value=["T1", "T1-1"],
			),
			patch(
				"erpnext.accounts.report.general_ledger.general_ledger.get_cost_centers_with_children",
				return_value=["_Test Cost Center - _TC"],
			),
			patch(
				"frappe.get_cached_value",
				side_effect=lambda dt, name, field: 1
				if (dt, name, field) == ("DocType", "TreeDoc", "is_tree")
				else 0,
			),
		):
